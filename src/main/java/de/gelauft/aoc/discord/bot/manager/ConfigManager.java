...
import de.gelauft.aoc.discord.bot.objects.configuration.*;
import de.gelauft.aoc.discord.bot.objects.permission.MasterRole;
        ...
        if (!file.exists()) {
            roleConfigObject = RoleConfigObject.builder()
                    .permissionGroups(Arrays.asList(
                            PermissionGroup.builder().groupName("cpt").roleId(1234L).build(),
                            PermissionGroup.builder().groupName("1lt").roleId(654654L).build(),
                            PermissionGroup.builder().groupName("lt").roleId(1234L).build(),
                            PermissionGroup.builder().groupName("smj").roleId(1234L).build(),
                            PermissionGroup.builder().groupName("sgt").roleId(1234L).build()))
    ...
    public ChannelConfigObject loadChannelConfig() {
        ...
        if (!file.exists()) {
            channelConfigObject = ChannelConfigObject.builder().configChannels(Arrays.asList(
                            ...
                            ConfigChannel.builder().simplifiedName("jfc").channelId(1234L).build()))
